...
def get_data():
    ...
    for i in range(1, 4):
        with open(f'info_{i}.txt', 'rb') as file:
            ...
            file_data = file_read.decode(result['encoding'])

        os_prog_data = re.compile(r'Изготовитель системы:\s+\S+')
        os_prod_list.append(os_prog_data.findall(file_data)[0].split()[2])

        os_name_data = re.compile(r'Название ОС:\s+\S+\s+\S+\s+\S+\s+\S+')
        os_name_list.append(' '.join(os_name_data.findall(file_data)[0].split()[2:]))

        os_code_data = re.compile(r'Код продукта:\s+\S+')
        os_code_list.append(os_code_data.findall(file_data)[0].split()[2])

        os_type_data = re.compile(r'Тип системы:\s+\S+')
        os_type_list.append(os_type_data.findall(file_data)[0].split()[2])
